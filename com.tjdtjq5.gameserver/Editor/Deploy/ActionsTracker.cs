...
namespace Tjdtjq5.GameServer.Editor
{
    ...
    public static class ActionsTracker
    {
        ...
        static void FetchCloudRunUrl()
        {
            ...
            if (!gcloud.Installed)
            {
                Debug.LogWarning("[GameServer] gcloud 미설치 — Cloud Run URL 자동 저장 불가");
                return;
            }
            ...
            System.Threading.Tasks.Task.Run(() =>
            {
                ...
                EditorApplication.delayCall += () =>
                {
                    if (code == 0 && !string.IsNullOrEmpty(output?.Trim()))
                    {
                        ...
                        settings.Save();
                        Debug.Log($"[GameServer] Cloud Run URL 저장: {CloudRunUrl}");
                    }
                    else
                    {
                        Debug.LogWarning($"[GameServer] Cloud Run URL 조회 실패 (code={code})");
                    }
                };
            });
        }
