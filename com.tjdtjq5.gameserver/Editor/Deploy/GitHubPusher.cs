...
namespace Tjdtjq5.GameServer.Editor
{
    ...
    {
        public static void Push(GameServerSettings settings, List<GeneratedFile> files,
            Action onSuccess, Action<string> onFailed)
        {
            ...
            {
                // 1. clone
                Debug.Log($"[GameServer:Deploy] Cloning {gh.Account}/{repoName}...");
                ...
                Debug.Log($"[GameServer:Deploy] {files.Count} files written");
                ...
                var (commitCode, commitOut) = PrerequisiteChecker.Run("git",
                    $"-C \"{tempDir}\" commit -m \"deploy from Unity GameServer\"");
                ...
                if (commitOut.Contains("nothing to commit"))
                {
                    ...
                    Debug.Log("[GameServer:Deploy] 코드 변경 없음, 시크릿 갱신 후 재배포");
                ...
                Debug.Log("[GameServer:Deploy] Push 완료!");
                onSuccess?.Invoke();
            }
            catch (Exception ex)
            {
        ...
        static void SetSecrets(GameServerSettings settings, PrerequisiteChecker.ToolStatus gh)
        {
            var repo = $"{gh.Account}/{settings.githubRepoName}";
            var projectId = settings.SupabaseProjectId;
            var dbPassword = GameServerSettings.SupabaseDbPassword;

            // Supabase 연결 문자열
            var connStr = $"Host=db.{projectId}.supabase.co;Port=5432;Database=postgres;Username=postgres;Password={dbPassword}";
            ...
            Debug.Log("[GameServer:Deploy] GitHub Secrets 설정 완료");
        }
