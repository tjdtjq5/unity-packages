...
namespace Tjdtjq5.GameServer.Editor
{
    public static class DeployManager
    {
        ...
        public static (List<GeneratedFile> files, Type[] logicTypes, string error) GenerateFiles(
            GameServerSettings settings, Action<string> onProgress = null)
        {
            ...
            Debug.Log($"[GameServer:Deploy] 스캔 완료 — Table: {tableTypes.Length}, Config: {specTypes.Length}, Service: {logicTypes.Length}");
            ...
            Debug.Log($"[GameServer:Deploy] 파일 {files.Count}개 생성");
            return (files, logicTypes, null);
        }

        // ── 배포 ──

        public static void Deploy(GameServerSettings settings,
            ...
            Action onSkipped = null)
        {
            ...
            GitHubPusher.Push(settings, files,
                onSuccess: () =>
                {
                    ...
                    Debug.Log($"[GameServer:Deploy] 배포 목록 업데이트: {endpoints.Count}개 엔드포인트");
                    onSuccess?.Invoke();
                },
                onFailed: error =>
                {
        ...
        public static (string tempDir, string error) PrepareBuildTest(GameServerSettings settings)
        {
        ...
        static List<GeneratedFile> GetTemplateFiles(GameServerSettings settings)
        {
            ...
            files.Add(LoadTemplate(templateRoot, "AspNetTemplate~/GameServer.csproj.template", "GameServer.csproj", settings));
        ...
        static GeneratedFile LoadTemplate(string root, string templatePath, string outputPath, GameServerSettings settings)
        {
