...
namespace Tjdtjq5.GameServer.Editor
{
    public static class ServerCodeGenerator
    {
        ...
        static GeneratedFile GenerateAttributeStubs()
        {
            ...
            sb.AppendLine("namespace Tjdtjq5.GameServer");
            sb.AppendLine("{");

            string[] attrs = { "Table", "Config", "Service", "API",
        ...
        static GeneratedFile GenerateLogicController(Type type)
        {
            ...
            foreach (var m in methods)
            {
                ...
                if (m.ReturnType.IsGenericType)
                    sb.AppendLine($"            var result = await service.{m.Name}({args});");
                else
                    sb.AppendLine($"            await service.{m.Name}({args});");

                sb.AppendLine(m.ReturnType.IsGenericType
                    ? "            return Ok(result);"
                    : "            return Ok();");
                sb.AppendLine("        }");
                ...
                sb.AppendLine("            return StatusCode(500, new { error = ex.Message, stack = ex.StackTrace });");
                sb.AppendLine("        }");
                sb.AppendLine("    }");
            }
