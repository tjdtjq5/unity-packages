...
namespace Tjdtjq5.GameServer.Editor
{
    public class DeployTab
    {
        ...
        public DeployTab(GameServerDashboard dashboard) => _dashboard = dashboard;
        ...
        void SyncTrackerState(GameServerSettings settings)
        {
        ...
        void DrawCacheSection(GameServerSettings settings)
        {
        ...
        void DrawDeployArea(GameServerSettings settings)
        {
            EditorUI.BeginBody();
        ...
        void DrawIdle(GameServerSettings settings)
        {
            EditorUI.DrawDescription(
                "[배포] 클릭 시:\n" +
        ...
        void DrawTracking(GameServerSettings settings)
        {
        ...
        void DrawBuildSuccess(GameServerSettings settings)
        {
        ...
        void DrawBuildFailed(GameServerSettings settings)
        {
        ...
        enum DbSyncState { Idle, Running, Success, Failed }
        DbSyncState _dbSyncState;
        string _dbSyncMessage;

        void DrawDbSync(GameServerSettings settings)
        {
            var hasToken = !string.IsNullOrEmpty(GameServerSettings.SupabaseAccessToken);
            if (!hasToken) return; // Access Token 없으면 표시 안 함

            EditorUI.DrawSubLabel("DB 동기화");
            EditorUI.BeginBody();
            EditorUI.DrawDescription(
                "서버 배포 없이 [Table]/[Config]의 마이그레이션 SQL을\nSupabase DB에 직접 실행합니다.",
                EditorUI.COL_MUTED);
            ...
            switch (_dbSyncState)
            {
                case DbSyncState.Running:
                    EditorUI.DrawLoading(true, "SQL 실행 중...");
                    break;
                case DbSyncState.Success:
                    EditorUI.DrawDescription($"✓ {_dbSyncMessage}", EditorUI.COL_SUCCESS);
                    GUILayout.Space(4);
                    if (EditorUI.DrawColorButton("다시 실행", EditorUI.COL_INFO, 24))
                        RunDbSync(settings);
                    break;
                case DbSyncState.Failed:
                    EditorUI.DrawDescription($"✗ {_dbSyncMessage}", EditorUI.COL_ERROR);
                    GUILayout.Space(4);
                    if (EditorUI.DrawColorButton("다시 실행", EditorUI.COL_ERROR, 24))
                        RunDbSync(settings);
                    break;
                default:
                    if (EditorUI.DrawColorButton("DB만 동기화", EditorUI.COL_INFO, 28))
                        RunDbSync(settings);
                    break;
            }
        ...
        async void RunDbSync(GameServerSettings settings)
        {
            _dbSyncState = DbSyncState.Running;
            _dashboard.Repaint();

            try
            {
                // 마이그레이션 SQL 생성
                var sql = ServerCodeGenerator.GenerateMigrationSql();
                if (string.IsNullOrEmpty(sql))
                {
                    _dbSyncState = DbSyncState.Failed;
                    _dbSyncMessage = "[Table]/[Config] 클래스를 찾을 수 없습니다";
                    _dashboard.Repaint();
                    return;
                }

                var (ok, result, error) = await SupabaseManagementApi.RunQuery(
                    settings.SupabaseProjectId, GameServerSettings.SupabaseAccessToken, sql);

                if (ok)
                {
                    _dbSyncState = DbSyncState.Success;
                    _dbSyncMessage = "마이그레이션 완료";
                }
                else
                {
                    _dbSyncState = DbSyncState.Failed;
                    _dbSyncMessage = error;
                }
            }
            catch (System.Exception ex)
            {
                _dbSyncState = DbSyncState.Failed;
                _dbSyncMessage = ex.Message;
            }
        ...
        void RunDeploy(GameServerSettings settings)
        {
            // dotnet 미설치 경고
        ...
        void DoDeploy(GameServerSettings settings)
        {
