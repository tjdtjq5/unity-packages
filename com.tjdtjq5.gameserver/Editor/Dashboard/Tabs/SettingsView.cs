...
namespace Tjdtjq5.GameServer.Editor
{
    public class SettingsView
    {
        ...
        void DrawSupabaseCard(GameServerSettings settings)
        {
            ...
            var expanded = EditorUI.BeginServiceCard("Supabase", GameServerDashboard.COL_SUPABASE,
            ...
            if (expanded)
            {
                ...
                {
                    if (EditorUI.DrawColorButton("프로젝트 조회", GameServerDashboard.COL_SUPABASE))
                ...
                {
                    // 읽기전용 표시
                    var anonDisplay = string.IsNullOrEmpty(GameServerSettings.SupabaseAnonKey)
                        ? "(프로젝트 선택 시 자동 조회)"
                        : GameServerSettings.SupabaseAnonKey.Length > 20
                            ? GameServerSettings.SupabaseAnonKey.Substring(0, 20) + "..."
                            : GameServerSettings.SupabaseAnonKey;
                    EditorUI.DrawCellLabel($"  Anon Key: {anonDisplay}", 0,
                        string.IsNullOrEmpty(GameServerSettings.SupabaseAnonKey) ? EditorUI.COL_MUTED : EditorUI.COL_SUCCESS);
                }
                else
                {
                    var anonKey = EditorUI.DrawTextField("Anon Key", GameServerSettings.SupabaseAnonKey, "수동 입력");
                ...
                // ── DB Password ──
                GUILayout.Space(2);
                var dbPw = EditorUI.DrawPasswordField("DB Password", GameServerSettings.SupabaseDbPassword, "프로젝트 생성 시 비밀번호");
            ...
            GUILayout.Space(4);
            EditorUI.BeginRow();
            if (EditorUI.DrawColorButton("연결 테스트", GameServerDashboard.COL_SUPABASE))
        ...
        void DrawGitHubCard(GameServerSettings settings, PrerequisiteChecker.ToolStatus gh)
        {
            ...
            EditorUI.BeginServiceCard("GitHub", GameServerDashboard.COL_GITHUB,
        ...
        void DrawProviderGuide(GameServerSettings settings, string providerKey, GuideInfo guide)
        {
            EditorUI.BeginBody();
        ...
        void DrawProviderCompleted(GameServerSettings settings, string providerKey, GuideInfo guide)
        {
        ...
        void DrawAutoProviderSetup(GameServerSettings settings, string providerKey, GuideInfo guide, string projectId)
        {
            ...
            else
            {
                ...
                {
                    if (EditorUI.DrawColorButton("Supabase에 적용", GameServerDashboard.COL_SUPABASE, 28))
        ...
        void DrawGpgsAutoGuide(GameServerSettings settings, string providerKey, GuideInfo guide, string projectId)
        {
            // Step 1~3: 기존 가이드
        ...
        async void ApplyProviderToSupabase(GameServerSettings settings, string providerKey)
        {
        ...
        void DrawAuthUrlSection(GameServerSettings settings)
        {
        ...
        void DrawGcpCard(GameServerSettings settings, PrerequisiteChecker.ToolStatus gcloud)
        {
            ...
            EditorUI.BeginServiceCard("GCP", GameServerDashboard.COL_GCP,
        ...
        async void FetchAuthConfig(GameServerSettings settings)
        {
            _authConfigLoading = true;
            var (ok, json, _) = await SupabaseManagementApi.GetAuthConfig(
                settings.SupabaseProjectId, GameServerSettings.SupabaseAccessToken);
        ...
        async void FetchAnonKey(GameServerSettings settings)
        {
        ...
        async void RunConnectionTest(GameServerSettings settings)
        {
            var token = GameServerSettings.SupabaseAccessToken;
        ...
        async void EnableProviderOnSupabase(GameServerSettings settings, string provider)
        {
            var token = GameServerSettings.SupabaseAccessToken;
            ...
            if (ok)
                Debug.Log($"[GameServer] {provider} Supabase에 자동 활성화됨");
            else
                Debug.LogWarning($"[GameServer] {provider} 활성화 실패: {error}");
        }

        /// <summary>Provider를 Supabase에서 비활성화.</summary>
        async void DisableProviderOnSupabase(GameServerSettings settings, string provider)
        {
            var token = GameServerSettings.SupabaseAccessToken;
            if (string.IsNullOrEmpty(token)) return;
            ...
            if (ok)
                Debug.Log($"[GameServer] {provider} Supabase에서 비활성화됨");
        }

        // ── 서버 로그 ──

        void DrawLogSection(GameServerSettings settings)
        {
