...
namespace Tjdtjq5.GameServer.Editor
{
    ...
    public static class GcpSetupUI
    {
        ...
        static Phase GetPhase(PrerequisiteChecker.ToolStatus gcloud, GameServerSettings s)
        {
        ...
        static void DrawLogin()
        {
            ...
            GUILayout.Space(4);
            if (EditorUI.DrawColorButton("로그인", GameServerDashboard.COL_GCP, 28))
                PrerequisiteChecker.RunGcloudLogin();
        }

        static void DrawProjectSelector(GameServerSettings settings)
        {
        ...
        static void DrawConfigAndAutoSetup(GameServerDashboard dashboard,
            GameServerSettings settings, PrerequisiteChecker.ToolStatus gcloud)
        {
            using (var so = new SerializedObject(settings))
            {
                so.Update();
            ...
            {
                if (EditorUI.DrawColorButton("자동 설정 시작", GameServerDashboard.COL_GCP, 32))
                {
