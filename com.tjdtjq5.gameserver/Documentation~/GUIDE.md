...
GameServer 패키지를 사용하면 서버 코드를 Unity 안에서 작성하고,
...
public class Player
{
    ...
    // ── 보안 ──
    [ServerOnly] public string secretData; // 클라이언트에 노출 안 됨
...
```csharp
// 단건 조회
var result = await GameServer.Get<Player>("player_001");
if (result.success)
    Debug.Log($"닉네임: {result.data.nickname}");

// 조건 검색 (Source Generator 자동 생성)
var topPlayers = await ServerProxy.QueryPlayers(q => q
    .LevelGreaterThan(50)
    .OrderByGoldDesc()
    .Limit(10)
);
...
```csharp
// 전체 조회
var allItems = await GameServer.GetAll<ShopItem>();
foreach (var item in allItems.data)
    Debug.Log($"{item.name}: {item.price}G");

// 단건 조회
var sword = await GameServer.Get<ShopItem>("sword_01");

// 조건 검색
var expensive = await ServerProxy.QueryShopItems(q => q
    .PriceGreaterThan(500)
    .OrderByPriceAsc()
);
```

### [TableData] vs [SpecData] 차이

| | [TableData] | [SpecData] |
|---|---|---|
| 용도 | 플레이어 데이터 (변동) | 게임 설정 (고정) |
| 예시 | Player, Item, GuildMember | ShopItem, LevelTable |
| 누가 수정 | [ServerLogic]에서만 | 개발자 (대시보드에서) |
| 클라이언트 | 읽기만 | 읽기만 |
| 캐싱 | 매번 조회 | 서버 메모리 캐싱 |
...
데이터를 **수정**하는 모든 작업은 [ServerLogic]으로 작성합니다.
...
Source Generator가 자동으로 `ServerProxy.PlayerService`를 생성합니다.

```csharp
// 타입 안전 프록시 (자동완성 지원)
var result = await ServerProxy.PlayerService.CreatePlayer("p1", "홍길동");
if (result.success)
    Debug.Log($"생성 완료: {result.data.nickname} Lv.{result.data.level}");

await ServerProxy.PlayerService.ChangeNickname("p1", "새닉네임");
```
...
```csharp
var result = await ServerProxy.ShopService.BuyItem("p1", "sword_01");
if (result.success)
...
```
1. [TableData], [SpecData] 클래스 작성
2. [ServerLogic] 클래스 작성
3. Unity Play 클릭
4. ServerProxy.XXX.Method() 호출
...
```
ServerProxy.ShopService.BuyItem()    → 배포됨 → 서버 호출
ServerProxy.ShopService.SellItem()   → 미배포 → 로컬 실행 (방금 작성)
ServerProxy.PlayerService.Create()   → 배포됨 → 서버 호출
ServerProxy.MatchService.FindMatch() → 미배포 → 로컬 실행
...
| 어트리뷰트 | 대상 | 설명 |
|-----------|:----:|------|
| `[TableData]` | class | 게임 데이터 테이블 (플레이어 데이터 등) |
| `[SpecData]` | class | 설정 데이터 (상점 가격, 레벨 테이블 등) |
| `[ServerLogic]` | class | 서버 로직 (데이터 수정하는 비즈니스 로직) |

### 메서드 어트리뷰트

| 어트리뷰트 | 대상 | 설명 |
|-----------|:----:|------|
| `[ServerMethod]` | method | API로 노출할 메서드. 생략 시 모든 public 메서드 대상 |
...
| `[ServerOnly]` | 클라이언트 비노출 | `[ServerOnly] public string hash;` |
...
```
Assets/Scripts/
├── TableData/           ← [TableData] 클래스
│   ├── Player.cs
│   ├── Item.cs
│   └── GuildMember.cs
├── SpecData/            ← [SpecData] 클래스
│   ├── ShopItem.cs
│   └── LevelTable.cs
├── ServerLogic/         ← [ServerLogic] 클래스
│   ├── PlayerService.cs
│   ├── ShopService.cs
│   └── MatchService.cs
└── Game/                ← 게임 코드
