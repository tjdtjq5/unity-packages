...
namespace Tjdtjq5.SupaRun
{
    ...
    public class GPGSAuthHandler : IPlatformAuth
    {
        ...
        public async Task<string> GetToken()
        {
            ...
            {
                Debug.LogWarning("[SupaRun:Auth] GPGS SDK가 설치되지 않았습니다.");
                return null;
            }
            ...
            try
            {
                ...
                if (instance == null)
                {
                    Debug.LogWarning("[SupaRun:Auth] GPGS Instance null");
                    return null;
                }
                ...
                if (signInStatusType != null)
                {
                    ...
                    {
                        Debug.LogWarning("[SupaRun:Auth] GPGS Authenticate(Action<SignInStatus>) 메서드를 찾을 수 없습니다.");
                        return null;
                    }
                }
                else
                {
                    ...
                    {
                        Debug.LogWarning("[SupaRun:Auth] GPGS Authenticate 메서드를 찾을 수 없습니다.");
                        return null;
                    }
                }
                ...
                if (!loggedIn)
                {
                    Debug.LogWarning("[SupaRun:Auth] GPGS 로그인 실패");
                    return null;
                }
                ...
                {
                    Debug.LogWarning("[SupaRun:Auth] RequestServerSideAccess 메서드를 찾을 수 없습니다.");
                    return null;
                }

                var authCode = await codeTcs.Task;
                if (string.IsNullOrEmpty(authCode))
                    Debug.LogWarning("[SupaRun:Auth] GPGS Server Auth Code 획득 실패");

                return authCode;
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[SupaRun:Auth] GPGS 에러: {ex.Message}");
                return null;
            }
        }
        ...
        static Action<T> MakeTypedCallback<T>(object successValue, TaskCompletionSource<bool> loginTcs)
        {
            ...
            return status =>
            {
                Debug.Log($"[SupaRun:Auth] GPGS Authenticate 결과: {status}");
