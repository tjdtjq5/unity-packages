...
namespace Tjdtjq5.Codemagic.Editor.Setup.Steps
{
    ...
    public sealed class Step1PreflightStep : ISetupStep
    {
        ...
        {
            EditorGUILayout.LabelField("Step 1/6: 사전 체크", EditorStyles.miniLabel);
            ...
            {
                EditorGUILayout.LabelField("검사 결과가 없습니다.", EditorStyles.wordWrappedMiniLabel);
            }
            else
            {
                foreach (var item in _lastResult)
                {
                    ...
                    EditorGUILayout.LabelField($"  {mark} {item.Name}");
                    if (!string.IsNullOrEmpty(item.Detail))
                        EditorGUILayout.LabelField($"      {item.Detail}",
                            EditorStyles.wordWrappedMiniLabel);
                }
            }
            ...
            if (_lastResult != null)
            {
                ...
                if (hasFail)
                {
                    GUILayout.Space(4);
                    EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                    EditorGUILayout.LabelField("해결 가이드");
                    GUILayout.Space(2);
                    EditorGUILayout.LabelField(
